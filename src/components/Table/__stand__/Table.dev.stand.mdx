...
import { TableExampleOnScrollToBottom } from './examples/TableExampleOnScrollToBottom/TableExampleOnScrollToBottom';
import { TableExampleOnAfterResize } from './examples/TableExampleOnAfterResize/TableExampleOnAfterResize';
...
| [`getRowKey?`](#ключ-строки)                 | `GetRowKey<ROW>`              | `(row) => row.id` | Функция получения ключа, если ключ не найден берется `index` |
| [`onRowMouseEnter?`](#выделение-строк)       | `TableRowMouseEvent<ROW>`     | -                 | Событие `onMouseEnter` на строке                             |
| [`onRowMouseLeave?`](#выделение-строк)       | `TableRowMouseEvent<ROW>`     | -                 | Событие `onMouseLeave` на строке                             |
| [`onRowClick?`](#выделение-строк)            | `TableRowMouseEvent<ROW>`     | -                 | Событие `onClick` на строке                                  |
...
| [`onAfterResize?`](#onafterresize)           | `TableResizeEvent`            | -                 | Событие завершения изменения ширины колонки (`pointerup`)    |
...
Свойство `onAfterResize` позволяет узнать финальную ширину колонок после того, как пользователь закончил перетаскивание ручки изменения размера (`pointerup`/`touchend`). Полезно, чтобы сохранить пользовательскую раскладку колонок (например, в `localStorage` или на бэкенде) и восстановить её при следующем открытии таблицы.
...
В обработчик передаётся массив со всеми колонками, у которых есть `accessor` и измеримая ширина, — не только той, которую двигал пользователь. Это важно в режиме `resizable="inside"`: при изменении одной колонки таблица может пропорционально изменить ширину соседних, чтобы вписаться в контейнер, поэтому в `onAfterResize` попадает актуальное состояние всех колонок целиком.

<MdxInformer status="normal">

`onAfterResize` не вызывается во время самого перетаскивания (на каждый `pointermove`), а только один раз — по его завершении. Это осознанное решение: применять новую ширину колонки к `columns` "на лету" (при каждом `pointermove`) не нужно и не имеет смысла, так как обновлённое значение будет применено таблицей автоматически без явной синхронизации с внешним состоянием.

</MdxInformer>

<MdxTabs>

<TableExampleOnAfterResize />
...
const baseColumns: TableColumn<ROW>[] = [
  { title: 'Атлет', accessor: 'athlete', width: 'auto', minWidth: 160 },
  { title: 'Возраст', accessor: 'age', width: 'auto', minWidth: 100 },
  { title: 'Страна', accessor: 'country', width: 'auto', minWidth: 160 },
];
...
export const TableExampleOnAfterResize = () => {
  // Сохранённая ширина колонок, например, полученная из localStorage/бэкенда
  ...
  const handleAfterResize = useCallback((result: TableColumnResizeResult[]) => {
    ...
    // Здесь же можно отправить `result` на бэкенд/в localStorage,
    // чтобы восстановить раскладку при следующем визите пользователя
  }, []);
...
<TableExampleLoadingDataWithLoader />
...
<TableExampleLoadingDataWithSkeleton />
...
<TableExampleLoadingRowWithLoader />
...
<TableExampleLoadingRowWithSkeleton />
...
<TableExampleLoadingNestedRowWithLoader />
