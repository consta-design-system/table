...
Для обединения ячеек используйте свойство `colmuns.colSpan`. Результат функции должен вернуть `number` | `'end'` | `undefined`.

- `number` - количество колонок занимаемое ячейкой.
- `'end'` - ячейка займет все соледующие столбцы до конца строки.
- `undefined` - ячейка замймет одну колонку(эквивалентно `1`)

> Если используете [закрепление колонок](#закрепление-колонок), cледите за тем чтобы объедененные ячеки были либо не закрепленные либо все закрепленные с одной стороны.
