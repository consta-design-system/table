...
- [Выделение строк](#выделение-строк)
  - [Наведение на строку](#наведение-на-строку)
    ...
    - [Подконтрольный способ](#подконтрольный-способ)
  - [Активность строки](#активность-строки)
  - [Пример с наведением и активацией строки](#пример-с-наведением-и-активацией-строки)
...
Если в таблице много данных, включите свойство `virtualScroll` и ограничьте высоту таблицы для лучшей производительности. Может принимать значение `boolean | [boolean, boolean]`. Если указывать тип `boolean`, то будет включен виртуальный скролл по горизонтали и вертикали. При указании типа `[boolean, boolean]`, первый элемент определяет, включается ли виртуальный скролл по горизонтали, второй – по вертикали.

<MdxInformer>

Если включаете виртуальный скролл по горизонтали, то высота ячеек в строке должна быть одинаковая чтобы избежать скачков строки.
...
Пример со свойством `rowHoverEffect`:
...
#### Подконтрольный способ

Пример с атрибутом `data-row-hover`.
...
В примере ниже используется стейт-менеджер [reatom](https://www.reatom.dev/).
...
### Пример с наведением и активацией строки
...
Свойство `headerZIndex` влияет на [`z-index`](https://developer.mozilla.org/ru/docs/Web/CSS/z-index):
