import {
  getGroups,
  GetGroupsResult,
} from '@consta/uikit/__internal__/src/utils/getGroups';
import { useCreateAtom } from '@consta/uikit/__internal__/src/utils/state/useCreateAtom';
import {
  usePickAtom,
  usePropAtom,
} from '@consta/uikit/__internal__/src/utils/state/usePickAtom';
import { useClickOutside } from '@consta/uikit/useClickOutside';
import { useDebounce } from '@consta/uikit/useDebounce';
import { KeyHandlers, useKeysRef } from '@consta/uikit/useKeysRef';
...
import { FilterGroupDefault, FilterItemDefault, FilterPropOnChange } from '..';
import { PropsWithDefault } from '../defaultProps';
import { CountedGroup, FilterAllItem, Group } from '../types';
...
export type OptionProps<ITEM> = {
  index: number;
  item: ITEM | OptionForCreate | FilterAllItem;
};
...
const isMultipleParams = <ITEM = FilterItemDefault, GROUP = FilterGroupDefault>(
...
export const isOptionForSelectAll = <ITEM>(
  params: FilterAllItem | ITEM,
): params is FilterAllItem => {
  return (
...
export const useFilter = <
  ITEM = FilterItemDefault,
  GROUP = FilterItemDefault,
  MULTIPLE extends boolean = false,
>({
  propsAtom,
}: UseFilterProps<ITEM, GROUP, MULTIPLE>) => {
  ...
  const onChange = useAction((ctx, e: React.SyntheticEvent, item: ITEM) => {
    ...
    if (multiple) {
      ...
      (onChange as FilterPropOnChange<ITEM, true>)(val, { e });
    } else {
      (onChange as FilterPropOnChange<ITEM, false>)(item, { e });
      onInput('');
      openAtom(ctx, false);
    }
  });
    ...
    (ctx, e: React.SyntheticEvent, items: ITEM[]) => {
      ...
      if (multiple) {
        ...
        if (currentGroupValues.length === nonDisabledItems.length) {
          (onChange as FilterPropOnChange<ITEM, true>)(withoutGroupValues, {
            e,
          });
        } else {
          const val = [...withoutGroupValues, ...nonDisabledItems];
          (onChange as FilterPropOnChange<ITEM, true>)(val, {
            e,
          });
        }
      }
  ...
  const Enter = useAction((ctx, e: React.SyntheticEvent) => {
    ...
    if (open) {
      ...
      const getData = (
        ...
        FilterAllItem | OptionForCreate | ITEM | undefined,
      ] => {
        let counter = 0;
