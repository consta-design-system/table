...
- `accessor`, указатель содержимого (должен быть уникальным для каждой колонки)
...
Для лучшей опитимизации рендера укажите уникальный ключ строки в `getRowKey`, если не указывать компонент попытается взять `row.id`, если же и там ни чего нет то будет использовать index. При этом ключи ячеек будут брать ключ строки и добавлять к нему `accessor`.
