...
- Наведение на строку – `hovered`,
- Выбор строки - `activated`

### Наведение на строку

Есть 2 способа задать состояние `hovered` строкам:

- Автоматически, задав свойство `rowHoverEffect`. В этом случае все строки будут подсвечиваться при наведении мыши.
- Подконтрольный, нужно у любой ячейки в строке указать атрибут `data-row-hover='true'`. этом режиме вы можете выбирать у каких строк включить этот эффект а у каких нет.
...
Пример со атрибутом `data-row-hover`.
