...
Для того чтобы указать какие строки у таблице являются выделенными используйте `getRowActive`
...
| `getRowActive?`    | ` (row: ROW, props: { index: number }) => boolean;` | -                 | Указатель выделения строки                                   |
