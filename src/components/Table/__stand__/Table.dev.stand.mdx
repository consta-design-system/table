...
import { TableExampleOnResize } from './examples/TableExampleOnResize/TableExampleOnResize';
...
| [`onResize?`](#onresize)                     | `TableResizeEvent`            | -                 | Событие завершения изменения ширины колонки (`pointerup`)    |
...
В обработчик передаётся массив со всеми колонками, у которых есть `accessor` и измеримая ширина, — не только той, которую двигал пользователь. Это важно в режиме `resizable="inside"`: при изменении одной колонки таблица может пропорционально изменить ширину соседних, чтобы вписаться в контейнер, поэтому в `onResize` попадает актуальное состояние всех колонок целиком.

<MdxInformer status="normal">

`onResize` не вызывается во время самого перетаскивания (на каждый `pointermove`), а только один раз — по его завершении. Это осознанное решение: применять новую ширину колонки к `columns` "на лету" (при каждом `pointermove`) не нужно и не имеет смысла, так как обновлённое значение будет применено таблицей автоматически без явной синхронизации с внешним состоянием.

</MdxInformer>

<MdxTabs>

<TableExampleOnResize />
