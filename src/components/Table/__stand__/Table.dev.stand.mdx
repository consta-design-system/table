...
Свойство `headerZIndex` влияет не только на `z-index` шапки таблицы, но и:

- У ячеек в закрепленных колонках 'z-index: calc(var(--table-header-z-index) - 1);'
- У ячеек в закрепленных колонках в шапке 'z-index: calc(var(--table-header-z-index) + 1);'
- У ресайзеров колонок `z-index: calc(var(--table-header-z-index) + 10);`

<MdxInformer
  status="normal"
  label="Если используете Popover или Tooltip внутри таблице привязывайте его z-index к --table-header-z-index, например:`z-index: calc(var(--table-header-z-index) - 1);` таким образом плавающий элемент будет находится под шапкой таблицы"
/>
