...
Для получения текущего отступа используйте свойство `onChange`. Для указания отступа используйте свойство `offset`.
...
Для получения текущего шага используйте свойство `onStepChange`. Для указания шага используйте свойство `step`.
...
Для того чтобы указать шаги используйте свойство `steps`. Шаги должны быть отсортированы по возрастанию. По умолчанию используются шаги `[10, 25, 50, 100]`.
...
Для того чтобы указать общее количество элементов используйте свойство `total`. Это свойство влияет на формат вывода текущей позиции (например `1-10 из 300`), на состояние `disabled` кнопки следующей страницы и на `onChange` не давая выйти за границы диапазона.
...
Для того чтобы указать текст слева компонента используйте свойство `label`. По умолчанию используется текст `'Строк на странице'`.
...
Для того чтобы указать текст текущей позиции используйте свойство `offsetLabel`.
...
Рекомендуется использовать компонент `Toolbar` для отображения компонента `Pagination`.
